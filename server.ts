...
async function persistStores(keys: string[]) {
  if (!dbPool || !keys.length) return;
  ...
  for (const key of [...new Set(keys)]) if (live[key]) await saveState(key, live[key]);
}
...
async function startServer() {
  ...
  app.post('/api/tenant-portal/facility-bookings', async (req:any,res)=>{const link=await linkedEntity(req,'tenant');if(!link)return res.status(404).json({message:'ملف المستأجر غير موجود'});const facility=facilitiesStore.find((x:any)=>String(x.id)===String(req.body?.facilityId||''));if(!facility)return res.status(400).json({message:'المرفق المحدد غير موجود'});const item={id:`booking-${crypto.randomUUID()}`,bookingNo:`BK-${Date.now()}`,tenantId:link.entityId,tenantName:link.entity.fullName||link.entity.name,unitNumber:link.entity.unitNumber||link.entity.houseNumber||'',facilityId:facility.id,facilityName:facility.name||facility.title||'',bookingDate:req.body?.bookingDate||'',startTime:req.body?.startTime||'',endTime:req.body?.endTime||'',guestsCount:Number(req.body?.guestsCount||0),purpose:req.body?.purpose||'',notes:req.body?.notes||'',status:'Pending',createdAt:new Date().toISOString()};facilityBookingsStore.unshift(item);await persistStores(['facilityBookings']);res.status(201).json(item);});
  app.put('/api/tenant-portal/facility-bookings/:id/cancel', async (req:any,res)=>{const link=await linkedEntity(req,'tenant');if(!link)return res.status(404).json({message:'ملف المستأجر غير موجود'});const item=facilityBookingsStore.find((x:any)=>String(x.id)===req.params.id&&String(x.tenantId||'')===link.entityId);if(!item)return res.status(404).json({message:'الحجز غير موجود'});if(item.status==='Cancelled')return res.status(409).json({message:'الحجز ملغي بالفعل'});item.status='Cancelled';item.cancelReason=req.body?.reason||'';item.cancelledAt=new Date().toISOString();item.updatedAt=item.cancelledAt;await persistStores(['facilityBookings']);res.json(item);});
  ...
  app.get('/api/tenant-portal/letters', async (req:any,res)=>{
    ...
    const visible=lettersStore.filter((x:any)=>{
      const rt=String(x.recipientType||'').trim().toLowerCase(); if(rt==='alltenants'||rt==='all tenants'||rt==='all_tenants') return true;
  ...
  app.get('/api/staff-portal/maintenance', async (req:any,res)=>{const link=await linkedEntity(req,'staff');if(!link)return res.status(404).json({message:'ملف الموظف غير موجود'});res.json(maintenanceStore.filter((x:any)=>{const assigned=String(x.assignedStaffId||x.assignedToId||'');return !assigned||assigned===link.entityId;}).map((x:any)=>({...x,assignedStaffId:x.assignedStaffId||x.assignedToId||undefined,assignedStaffName:x.assignedStaffName||x.assignedToName||undefined})));});
  app.put('/api/staff-portal/maintenance/:id/claim', async (req:any,res)=>{
    const link=await linkedEntity(req,'staff');
    if(!link)return res.status(404).json({message:'ملف الموظف غير موجود'});
    const item=maintenanceStore.find((x:any)=>String(x.id)===req.params.id);
    if(!item)return res.status(404).json({message:'طلب الصيانة غير موجود'});
    const assigned=String(item.assignedStaffId||item.assignedToId||'');
    if(assigned && assigned!==link.entityId)return res.status(409).json({message:'الطلب مسند بالفعل إلى موظف آخر'});
    item.assignedStaffId=link.entityId; item.assignedToId=link.entityId;
    item.assignedStaffName=link.entity.fullName||link.entity.name||''; item.assignedToName=item.assignedStaffName;
    if(String(item.status||'New')==='New') item.status='Assigned';
    item.updatedAt=new Date().toISOString();
    await persistStores(['maintenance']);
    res.json(item);
  });
  app.put('/api/staff-portal/maintenance/:id/status', async (req:any,res)=>{const link=await linkedEntity(req,'staff');if(!link)return res.status(404).json({message:'ملف الموظف غير موجود'});const item=maintenanceStore.find((x:any)=>String(x.id)===req.params.id&&String(x.assignedStaffId||x.assignedToId||'')===link.entityId);if(!item)return res.status(404).json({message:'طلب الصيانة غير موجود أو غير مسند إليك'});const requested=String(req.body?.status||''); const current=String(item.status||'New'); const terminal=['Done','Completed','Closed']; if(terminal.includes(current)) return res.status(409).json({message:'طلب الصيانة مكتمل بالفعل ولا يمكن تغييره من حساب الموظف'}); const allowedByCurrent:Record<string,string[]>={New:['In Progress'],Assigned:['In Progress'],Open:['In Progress'],'In Progress':['Done','Completed'],'InProgress':['Done','Completed']}; const allowed=allowedByCurrent[current]||[]; if(!allowed.includes(requested)) return res.status(400).json({message:'لا يمكن الانتقال من الحالة الحالية إلى الحالة المحددة'}); item.status=requested==='Completed'?'Done':requested;item.updatedAt=new Date().toISOString();await persistStores(['maintenance']);res.json(item);});
  app.put('/api/staff-portal/maintenance/:id/notes', async (req:any,res)=>{const link=await linkedEntity(req,'staff');if(!link)return res.status(404).json({message:'ملف الموظف غير موجود'});const item=maintenanceStore.find((x:any)=>String(x.id)===req.params.id&&String(x.assignedStaffId||x.assignedToId||'')===link.entityId);if(!item)return res.status(404).json({message:'طلب الصيانة غير موجود أو غير مسند إليك'});item.workNotes=req.body?.notes||'';item.notes=req.body?.notes||item.notes||'';item.updatedAt=new Date().toISOString();await persistStores(['maintenance']);res.json(item);});
  ...
  app.get('/api/staff-portal/facilities', async (_req,res)=>res.json(facilitiesStore.filter((x:any)=>x.isActive!==false)));
  app.get('/api/staff-portal/facility-bookings', async (_req,res)=>res.json(facilityBookingsStore));
  app.get('/api/staff-portal/letters', async (_req,res)=>res.json(lettersStore));
  ...
  app.post("/api/Maintenance", async (req:any, res:any) => {
    const body=req.body||{};
  ...
  app.post("/api/Facilities", async (req,res)=>{ const item={id:`facility-${crypto.randomUUID()}`,createdAt:new Date().toISOString(),...req.body}; facilitiesStore.unshift(item); await persistStores(["facilities"]); res.status(201).json(item); });
  app.put("/api/Facilities/:id", async (req,res)=>{ const i=facilitiesStore.findIndex((x:any)=>x.id===req.params.id); if(i<0) return res.status(404).json({message:"Facility not found"}); facilitiesStore[i]={...facilitiesStore[i],...req.body,updatedAt:new Date().toISOString()}; await persistStores(["facilities"]); res.json(facilitiesStore[i]); });
  ...
  app.post("/api/FacilityBookings", async (req,res)=>{ const id=`booking-${crypto.randomUUID()}`; const item={id, bookingNo:`BK-${id.slice(-8).toUpperCase()}`, createdAt:new Date().toISOString(), ...req.body}; facilityBookingsStore.unshift(item); await persistStores(["facilityBookings"]); res.status(201).json(item); });
  app.put("/api/FacilityBookings/:id", async (req,res)=>{ const i=facilityBookingsStore.findIndex((x:any)=>x.id===req.params.id); if(i<0) return res.status(404).json({message:"Booking not found"}); facilityBookingsStore[i]={...facilityBookingsStore[i],...req.body,updatedAt:new Date().toISOString()}; await persistStores(["facilityBookings"]); res.json(facilityBookingsStore[i]); });
